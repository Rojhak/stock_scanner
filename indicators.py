...
def _true_range(df: pd.DataFrame) -> pd.Series: # Expects df with 'high', 'low', 'close'
    ...
    tr1 = (HIGH - LOW).abs(); tr2 = (HIGH - prev_close).abs(); tr3 = (LOW - prev_close).abs()
    tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1, skipna=False)
    return tr

def atr(df: pd.DataFrame, window: int = 14) -> pd.Series:
    """Average True Range (ATR). Expects lowercase 'high', 'low', 'close'."""
    required_cols = ['high', 'low', 'close']
    if not all(col in df.columns for col in required_cols):
        log.error(f"ATR requires lowercase cols: {required_cols}. Found: {df.columns.tolist()}")
        return pd.Series(dtype=float, index=df.index).fillna(np.nan)
    try:
        tr = _true_range(df)
...
def rs_line(target_close: pd.Series, bench_close: pd.Series, smooth: Optional[int] = None) -> pd.Series:
    """Calculates Price Relative Strength line (target/benchmark)."""
    if not isinstance(target_close, pd.Series) or not isinstance(bench_close, pd.Series):
        log.error("RS Line inputs must be Series."); return pd.Series(dtype=float)
    try:
        aligned_target, aligned_bench = target_close.align(bench_close, join='inner')
        if aligned_bench.empty or aligned_target.empty:
            log.warning("RS Line: No overlapping data."); return pd.Series(dtype=float, index=target_close.index).reindex(target_close.index)
        ...
        log.error(f"Error calculating RS Line: {e}"); return pd.Series(dtype=float, index=target_close.index).fillna(np.nan)

# --- Other indicators (rs_new_high, atr_multiple, vcp_dry_up) remain the same ---
def rs_new_high(rs_series: pd.Series, lookback: int = 252) -> pd.Series:
    """Identifies where RS makes a new high over lookback period."""
    if not isinstance(rs_series, pd.Series): log.error("rs_new_high input Series."); return pd.Series(dtype=bool)
    if lookback <= 0: log.error("rs_new_high lookback positive."); return pd.Series(dtype=bool)
    try:
    ...
    except Exception as e: log.error(f"Error RS New High: {e}"); return pd.Series(dtype=bool, index=rs_series.index).fillna(False)

def atr_multiple(price: pd.Series, atr_series: pd.Series) -> pd.Series: # Assumes 'close' series
    """Calculates ATR as a percentage of price."""
    if not isinstance(price, pd.Series) or not isinstance(atr_series, pd.Series): log.error("atr_multiple inputs Series."); return pd.Series(dtype=float)
    try:
    ...
    except Exception as e: log.error(f"Error ATR Multiple: {e}"); return pd.Series(dtype=float, index=price.index).fillna(np.nan)

def vcp_dry_up(volume: pd.Series, lookback: int = 10, thresh: float = 0.4) -> pd.Series: # Assumes 'volume' series
    """Minervini-style Volume Dry-Up check."""
    if not isinstance(volume, pd.Series): log.error("vcp_dry_up input Series."); return pd.Series(dtype=bool)
    if lookback <= 0: log.error("vcp_dry_up lookback positive."); return pd.Series(dtype=bool)
    try:
        sum_2n = volume.rolling(lookback * 2, min_periods=lookback * 2).sum()
        sum_prev_n = sum_2n.shift(lookback); sum_recent_n = sum_2n - sum_prev_n
        ratio = _safe_divide(sum_recent_n, sum_prev_n, default=np.inf)
        is_dry = ratio < thresh
        return is_dry.fillna(False)
    except Exception as e: log.error(f"Error VCP Dry Up: {e}"); return pd.Series(dtype=bool, index=volume.index).fillna(False)
