...
def is_compressed(df: pd.DataFrame, index_loc: int, lookback: int = 10, threshold: float = 0.03) -> bool:
    ...
    # Ensure columns exist with expected Capitalized names
    close_col, atr_col = 'Close', 'ATR'
    if not all(c in df.columns for c in [close_col, atr_col]):
        log.debug(f"is_compressed check skipped: Missing {close_col} or {atr_col}")
        return False
    if index_loc < lookback: return False # Not enough data

    try:
        recent_df = df.iloc[index_loc - lookback + 1 : index_loc + 1]
        if recent_df.empty: return False

        # Calculate ATR percentage of Close for the period
        atr_percent = (recent_df[atr_col] / recent_df[close_col].replace(0, np.nan)).fillna(0)

        # Check if the *average* ATR percent over the lookback is below threshold
        is_comp = atr_percent.mean() < threshold
        log.debug(f"Compression check at {df.index[index_loc]}: ATR% mean={atr_percent.mean():.4f}, Threshold={threshold}, Result={is_comp}")
        return is_comp
    except Exception as e:
        log.warning(f"Error during compression check: {e}")
        return False

# --- Add other filter functions below, ensuring they use Capitalized column names ---

def has_volume_spike(df: pd.DataFrame, index_loc: int, lookback: int = 50, factor: float = 1.5) -> bool:
    """Checks if volume on the current bar is significantly higher than average."""
    vol_col = 'Volume'
    ...
    try:
        ...
        avg_volume = df[vol_col].iloc[index_loc - lookback : index_loc].mean()
...
def momentum_confirmed(df: pd.DataFrame, index_loc: int, lookback: int = 5) -> bool:
    """Checks if short-term momentum (e.g., price change) is positive."""
    close_col = 'Close'
...
def in_base(df: pd.DataFrame, index_loc: int, lookback: int = 50, threshold: float = 0.15) -> bool:
    ...
    try:
        recent_df = df.iloc[index_loc - lookback + 1 : index_loc + 1]
        period_high = recent_df[high_col].max()
        period_low = recent_df[low_col].min()
        # Check if high and low are valid and range is within threshold
...
def rsi_confirmed(df: pd.DataFrame, index_loc: int, threshold: float = 50) -> bool:
    """Checks if RSI is above a certain threshold."""
    rsi_col = 'RSI' # Expect Capitalized RSI column
