...
try:
    ...
    from .scorer import score_dataframe, score_symbol
    from .alert import dispatch
    from .indicators import atr, ma, ema, rs_line, rs_new_high # These now expect lowercase cols
...
def _calculate_required_indicators(df: pd.DataFrame, bench_close: Optional[pd.Series] = None) -> pd.DataFrame:
    ...
    """
    if df.empty: return df # Return empty if input is empty
    df_out = df.copy()

    # --- Standardize to Lowercase for Processing ---
    df_out.columns = [str(col).lower() for col in df_out.columns]
    log.debug(f"Calculating indicators for DataFrame. Lowercase columns: {df_out.columns.tolist()}")

    # Define required cols in lowercase
    ...
    if not all(col in df_out.columns for col in required_cols_lower):
         ...
         log.warning(f"Missing required OHLCV columns (lowercase): {missing_cols}. Indicator calculation may fail.")
    ...
    try:
        ...
        if atr_col not in df_out.columns:
            if all(c in df_out.columns for c in [high_col, low_col, close_col]) and not df_out[[high_col, low_col, close_col]].isnull().all().all():
                 ...
                 if not ohlc_data_for_atr.empty:
                      atr_series = atr(ohlc_data_for_atr, window=14) # Pass df with lowercase cols
                      df_out[atr_col] = atr_series.reindex(df_out.index)
        ...
        if bench_close is not None and not bench_close.empty and close_col in df_out.columns and not df_out[close_col].isnull().all():
             ...
             if rs_line_col not in df_out.columns:
                  ...
                  else: df_out[rs_line_col] = np.nan; log.debug("Aligned benchmark resulted in all NaNs for RS Line calc.")
            ...
            log.debug(f"Skipping RS_Line/RS_slope calculation (Benchmark valid: {bench_close is not None and not bench_close.empty}, Close col valid: {close_col in df_out.columns and not df_out[close_col].isnull().all()})")
            df_out[rs_line_col] = np.nan; df_out[rs_slope_col] = np.nan

        log.debug(f"Finished calculating indicators. Lowercase columns: {df_out.columns.tolist()}")

    except Exception as e:
        log.warning(f"Error during indicator calculation: {e}", exc_info=True)

    # --- Rename columns back to desired output format (e.g., Capitalized) ---
    rename_map = {
        'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close', 'volume': 'Volume',
        'adj close': 'Adj Close', 'ma10': 'MA10', 'ma20': 'MA20', 'ma50': 'MA50', 'ma200': 'MA200',
        'atr': 'ATR', 'rsi': 'RSI', 'rs_line': 'RS_Line', 'rs_slope': 'RS_slope'
    }
...
# Corrected _fetch_and_prepare_data with robust benchmark 'Close' finding
def _fetch_and_prepare_data(symbols: List[str], benchmark: str = "SPY", period: str = '2y', interval: str = '1d', universe_name: Optional[str] = None) -> Tuple[Optional[Dict[str, pd.DataFrame]], Optional[pd.Series]]:
    """Fetches price data for symbols and benchmark, prepares benchmark close."""
    log.info(f"Fetching price data for {len(symbols)} symbols (Universe: {universe_name or 'Unknown'}) and benchmark {benchmark}...")
    today = dt.date.today()
    start_date = today - dt.timedelta(days=CONFIG.get("cache_days", 730))
    if 'y' in period:
        try: years = int(period.replace('y','')); start_date = today - dt.timedelta(days=years*365)
        except ValueError: pass
    elif 'm' in period:
        try: months = int(period.replace('m','')); start_date = today - dt.timedelta(days=months*30)
        except ValueError: pass
    ...
    if bench_df is None or bench_df.empty:
        log.warning(f"Failed benchmark fetch ({benchmark}).")
    else:
        log.debug(f"Benchmark ({benchmark}) DataFrame columns: {bench_df.columns}")
        log.debug(f"Benchmark ({benchmark}) DataFrame head:\n{bench_df.head(2)}")

        # --- More Forgiving Check for 'Close' column ---
        close_col_found = None
        for col in bench_df.columns:
            col_str = str(col)
            col_str_lower = col_str.lower()
            if col_str == 'Close': close_col_found = col; log.debug(f"Found exact benchmark 'Close' column: {col}"); break
            if col_str_lower == 'close': close_col_found = col; log.debug(f"Found exact lowercase benchmark 'close' column: {col}"); break
        if close_col_found is None:
             for col in bench_df.columns:
                  if 'close' in str(col).lower():
                      close_col_found = col; log.debug(f"Found column containing 'close' (case-insensitive): {col}"); break

        if close_col_found is not None:
            try:
                bench_close = bench_df[close_col_found]
                if bench_close.isnull().all(): log.warning(f"Benchmark column ({close_col_found}) all NaNs."); bench_close = None
                elif not pd.api.types.is_numeric_dtype(bench_close): log.warning(f"Benchmark column ({close_col_found}) not numeric."); bench_close = None
                else: log.info(f"Successfully extracted benchmark Close series from column: {close_col_found}")
            except Exception as e: log.error(f"Error extracting benchmark column ({close_col_found}): {e}"); bench_close = None
        else:
            log.error(f"Could not find any column containing 'close' (case-insensitive) in benchmark {benchmark}.")
    ...
    if all_data is None or all_data.empty: log.error(f"Failed fetch for symbols in universe '{universe_name}'."); return None, bench_close
        ...
        for sym in successful_tickers:
            if sym in symbols:
                 try:
                     ...
                     else: log.warning(f"Symbol {sym} missing 'Close'. Columns: {df_sym.columns}")
         ...
         if 'Close' in df_sym.columns: data_dict[sym] = df_sym.dropna(subset=['Close'])
         else: log.warning(f"Single symbol {sym} missing 'Close'. Columns: {df_sym.columns}")
    else: log.warning(f"Unexpected data structure/empty result for universe '{universe_name}'. Cols: {all_data.columns}")
    log.info(f"Successfully prepared data for {len(data_dict)} symbols in universe '{universe_name}'.")
...
def _process_and_score_symbol(symbol: str, df: pd.DataFrame, bench_close: Optional[pd.Series]) -> Optional[pd.Series]:
    ...
    if df.empty or len(df) < required_rows: log.debug(f"Skipping {symbol}: Insufficient data ({len(df)} rows, need {required_rows})."); return None
    try:
        df_with_indicators = _calculate_required_indicators(df, bench_close) # Returns cols Capitalized
        if df_with_indicators['ATR'].isnull().all(): log.warning(f"ATR calculation failed for {symbol}, skipping scoring."); return None

        scored_results = score_dataframe(df_with_indicators) # Scorer expects Capitalized cols
        if not scored_results.empty:
            ...
            else: log.error(f"Unexpected result type from score_dataframe for {symbol}: {type(scored_results)}"); return None
            latest_result['symbol'] = symbol
            log.debug(f"Found valid setup for {symbol} on {latest_result['date']} with score {latest_result.get('score', np.nan):.2f}")
            return latest_result
        else: log.debug(f"No valid setup found for {symbol} after scoring."); return None
    except Exception as e: log.error(f"Error processing symbol {symbol}: {e}", exc_info=False); return None

# --- Public API ---
class LeadershipScanner:
    """High-level class to orchestrate the stock scanning process."""
    def __init__(self, universe: str = CONFIG.get("universe", "sp500"), benchmark: str = "SPY"):
        log.info(f"Initializing LeadershipScanner for universe '{universe}', benchmark '{benchmark}'.")
        self.universe_name = universe; self.benchmark_symbol = benchmark
        self.symbols: List[str] = []; self.data: Dict[str, pd.DataFrame] = {}
        self.benchmark_close: Optional[pd.Series] = None; self.results: Optional[pd.DataFrame] = None
        try: self.symbols = load_universe(self.universe_name)
        except ValueError as e: log.error(f"Failed to load universe '{self.universe_name}': {e}")
    ...
    def run(self, top: int = 20, min_score: float = 3.0, min_r: Optional[float] = None) -> pd.DataFrame:
        ...
        # Call fetch function and assign both returned values
        ...
        # *** Store the returned benchmark series in the class attribute ***
        self.benchmark_close = benchmark_close_series
        # *** REMOVED the redundant/incorrect line: self.benchmark_close = bench_close ***

        if not fetched_data: log.error("Data fetching failed. Aborting scan."); return pd.DataFrame()
        self.data = fetched_data; all_results = []
        log.info(f"Scoring {len(self.data)} symbols for universe '{self.universe_name}'...")
        for symbol, df in self.data.items():
             # Pass the potentially None self.benchmark_close
        ...
        try:
            ...
            if 'symbol' not in results_df.columns: log.warning("'symbol' column missing.")
...
def run_daily_scan(
    ...
    if leaders_df is not None and not leaders_df.empty:
        if not silent:
            ...
            if 'date' not in leaders_df.columns and isinstance(leaders_df.index, (pd.DatetimeIndex, pd.Index)):
                 leaders_df.reset_index(inplace=True)

            if 'Close' not in leaders_df.columns:
                 closes = []; date_col = 'date'
                 if date_col not in leaders_df.columns: log.error("Cannot find 'date' column for Close price lookup."); closes = [np.nan] * len(leaders_df)
                 else:
                     for idx in leaders_df.index:
                         row = leaders_df.loc[idx]; sym = row['symbol']
                         ...
                         except Exception as e: log.debug(f"Could not retrieve cached Close {sym} {event_date}: {e}"); closes.append(np.nan)
                 leaders_df['Close'] = closes
            final_display_cols = []
            for col in display_cols:
                if col in leaders_df.columns: final_display_cols.append(col)
                else: log.warning(f"Display column '{col}' missing, adding NA."); leaders_df[col] = pd.NA; final_display_cols.append(col)
        ...
        if alert:
            log.info("Dispatching alerts..."); today_str = dt.date.today().strftime('%Y-%m-%d')
            subject = f"Leader Scan ({today_str}) - {len(leaders_df)} Matches ({universe.upper()})"
            try:
                ...
                dispatch(subject, body); log.info("Alerts dispatched successfully.")
...
def run_scan_for_all_universes(top_per_universe: int = 5, alert: bool = False, benchmark: str = "SPY"):
    """Scans all CSV files in the resources directory, prints top results for each."""
    resources_dir = _PACKAGE_ROOT / "resources"
    if not resources_dir.is_dir(): log.error(f"Resources directory not found at {resources_dir}"); return
    ...
    for item in resource_files:
        ...
        except ValueError as ve: log.error(f"Skipping universe '{universe_name}' loading error: {ve}")
